...
function modidy_event_attendees!(event::Dict, attendees_ids::Vector{String})
    if attendees_ids !== []
        ids = []
        ...
        push!(event, "attendees" => ids)
    end
    return nothing
end
function modidy_event_attendees!(events::Vector{Dict}, attendees_ids::Vector{String})
    for event in events
        modidy_event_attendees!(event, attendees_ids)
    end
    return nothing
end

function generate_course_events(xlsxfile, course_start_date, attendees_ids)
    course_items = get_course_items(xlsxfile)
    events = deal_course_table(course_items, course_start_date)
    modify_course_repeat!(events)
    modidy_event_attendees!(events, attendees_ids)
    return events
end
